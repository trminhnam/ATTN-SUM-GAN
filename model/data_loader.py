...
class VideoData(Dataset):
    def __init__(self, mode, split_index):
        ...
        self.splits = []
        self.split_index = (
            split_index  # it represents the current split (varies from 0 to 4)
        )
        temp = {}
    ...
    def __getitem__(self, index):
        ...
        frame_features = torch.Tensor(
            np.array(self.video_data[video_name + "/features"])
        )
        if self.mode == "test":
...
def get_loader(mode, split_index, batch_size=1):
    if mode.lower() == "train":
        vd = VideoData(mode, split_index)
        return DataLoader(vd, batch_size=batch_size, shuffle=True)
    else:
