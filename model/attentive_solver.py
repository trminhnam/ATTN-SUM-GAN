...
from layers import AttentiveDiscriminator, AttentiveSummarizer
...
class AttentiveSolver(object):
    ...
    def build(self):
        ...
        ## Projection layers to fit the input size to the hidden size
        ...
        self.summarizer = AttentiveSummarizer(
        ...
        self.discriminator = AttentiveDiscriminator(
            d_model=self.config.hidden_size,
            nhead=self.config.nhead,
            num_layers=self.config.num_layers,
            dim_feedforward=self.config.dim_feedforward,
        ...
        if self.config.mode == "train":
            ...
            self.s_e_optimizer = optim.Adam(
                list(self.summarizer.attentive_selector.parameters())
                + list(self.summarizer.attentive_auto_encoder.encoder.parameters())
                + list(self.linear_compress.parameters()),
                lr=self.config.lr,
            )
            self.d_optimizer = optim.Adam(
                list(self.summarizer.attentive_auto_encoder.decoder.parameters())
                + list(self.linear_compress.parameters()),
                lr=self.config.lr,
            )
    ...
    def reconstruction_loss(self, h_origin, h_sum):
        """L2 loss between original-regenerated features at discriminator's last hidden layer"""
    ...
    def train(self):
        ...
        for epoch_i in trange(self.config.n_epochs, desc="Epoch", ncols=80):
            ...
            for batch_i, image_features in enumerate(
                ...
                # ---- Train selector, encoder ----#
                if self.config.verbose:
                    tqdm.write("\nTraining selector and encoder...")

                # [seq_len, 1, hidden_size]
                original_features = self.linear_compress(
                    image_features_.detach()
                ).unsqueeze(1)
                ...
                tqdm.write(
                    f"recon loss {reconstruction_loss.item():.3f}, sparsity loss: {sparsity_loss.item():.3f}"
                )
                ...
                wandb_log("recon_loss", reconstruction_loss.data)
                wandb_log("sparsity_loss", sparsity_loss.data)
                wandb_log("gen_loss", g_loss.data)

                wandb_log("original_prob", original_prob.data)
                wandb_log("sum_prob", sum_prob.data)
            ...
            ckpt_path = os.path.join(self.config.save_dir, f"epoch-{epoch_i}.pt")
    ...
    def evaluate(self, epoch_i):
        ...
        for video_tensor, video_name in tqdm(
            ...
            # [seq_len]
            with torch.no_grad():
                scores = self.summarizer.attentive_selector(video_feature).squeeze(1)
