...
class sLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=2):
        ...
        self.out = nn.Sequential(
            nn.Linear(hidden_size * 2, 1), nn.Sigmoid()  # bidirection => scalar
        )
...
class Attn(nn.Module):
    ...
    def Mult_score(self, prev_hidden, encoder_output):
        ...
        """
        prev_hidden = prev_hidden.transpose(
            0, 1
        )  # [batch_size=1, num_layers, hidden_size]
        prev_hidden = prev_hidden.reshape(
            prev_hidden.shape[0], -1
        )  # [batch_size=1, num_layers*hidden_size]
