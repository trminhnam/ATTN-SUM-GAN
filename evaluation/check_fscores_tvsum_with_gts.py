...
avg_across_5_folds = []
# result_path = "../model/output/lstm/base/tvsum/results"
# result_path = "../model/output/ablation_hidden_size/512/tvsum/results"
# result_path = "../model/output/ablation_num_head/2/tvsum/results"
result_path = "../model/output/lstm/base/tvsum/results"
for i in range(5):
    ...
    for epoch in results:
        ...
        all_gt_summary = generate_summary(
            all_shot_bound, all_gt_scores, all_nframes, all_positions
        )
        ...
        for video_index in range(len(all_summaries)):
            ...
            f_score = evaluate_summary(
                summary, np.expand_dims(gt_summary, axis=0), eval_method
            )
    ...
    print(
        f"Highest F-score: {np.round(np.max(f_score_epochs), decimals=2)} at epoch {np.argmax(f_score_epochs)}"
    )
...
with open(os.path.join(result_path, "avg_f_scores.txt"), "w") as outfile:
